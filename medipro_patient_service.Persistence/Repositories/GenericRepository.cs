...
public class GenericRepository<T> : IGenericRepository<T> where T : Entity
{
    ...
    public async Task<T?> FindAsync(Expression<Func<T, bool>> expression) =>
        await _dbSet.FirstOrDefaultAsync(expression ?? (entity => true));
    ...
    public async Task<PagedResult<T>> GetPagedResultAsync(int page, int pageSize, Expression<Func<T, bool>>? filter = null,
        ...
        params string[] includePropeeties)
    {
        IQueryable<T> query = _dbSet;
        if (filter is not null) query = query.Where(filter);
        query = includePropeeties.Aggregate(query, (current, includeProperty) => current.Include(includeProperty));
